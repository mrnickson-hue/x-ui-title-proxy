...
Причина — параметр `webDomain` в базе 3X-UI: он включает проверку хоста и блокирует запросы от прокси. Установщик удаляет его автоматически. Если устанавливали вручную — исправьте так:
```bash
systemctl stop x-ui
